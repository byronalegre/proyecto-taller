...
<head>
	...
	<body>
				...
				Reporte de Ordenes de Compra del año {{now()->format('Y')}}
				</h2>
				<div style="font-family: Tahoma, sans-serif; font-size: 10px;">Cantidad de items: {{ count($oc->all()) }}
				</div>
					...
					<tbody style="font-size: 12px; border-top: 1px solid black;">
						@foreach($oc as $i)
							<tr>
								<td>{{ $i->codigo }} </td>	
								<td>»</td>
								<td>{{ $i->orden->codigo }} </td><!--mirar esto-->			
								<td>{{ $i->provs->name }} </td>
