...
@section ('title','Información y Rol de usuario')
...
<li class="breadcrumb-item">
	<a href="{{url('/admin/usuarios/'.$u->id.'/edit') }}"><i class="fas fa-user-edit"></i> Editar usuario</a>
</li>
@endsection
					...
					<div class="inside">
						...
						<div>	
							@if(kvfj(Auth::user()->permisos, 'usuarios_suspend'))
								@if($u->status == "100")
									<a href="{{ url('admin/usuarios/'.$u->id.'/suspend')}}" class="btn btn-success"> Habilitar usuario
									</a>
								@else	
									<a href="{{ url('admin/usuarios/'.$u->id.'/suspend')}}" class="btn btn-danger"> Deshabilitar usuario
									</a>
								@endif
							@endif
						</div>
