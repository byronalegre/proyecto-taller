...
use Carbon\Carbon;
use App\Http\Models\Proveedor, App\Http\Models\OrdenCompra, App\Http\Models\Remito;
...
class ProveedoresController extends Controller
{
    ...
    public function getHome($status, Request $request){
        ...
        if($search){
            $provs = Proveedor::where('name','LIKE','%'.$search.'%')
            ...
            if($status == 'all'):
                $provs = Proveedor::orderBy('name', 'asc')->paginate($pag);
            elseif($status == 'trash'): 
                $provs = Proveedor::onlyTrashed()->orderBy('name', 'asc')->paginate($pag);
            endif;
        }
    	
    	$data = ['provs'=> $provs, 'status'=>$status, 'search'=>$search];
    ...
    public function getProveedorDelete($id){
        ...
        $cont_r = Remito::where('proveedor_id', $id)->count();
        $cont_oc = OrdenCompra::where('proveedor_id', $id)->count();

        if(($cont_r != 0)&&($cont_oc != 0))
        {
             return back()->with('message','No se puede Eliminar. Hay '.($cont_r + $cont_oc). ' elemento/s relacionado/s con este proveedor. ('.$cont_oc.' Ordenes de Compra - '.$cont_r.' Remitos).')->with('typealert','danger');
        }
        else
        {
        	if($c->delete()):
    ...
    public function getProveedorRestore($id){
     ...
     public function pdf(){
        ...
        $provs = Proveedor::orderBy('name','asc')->get(); 
    ...
    public function order($status, $campo, $direc, Request $request ){
        ...
        if($search){
            ...
            if($status == 'all'):
                $provs = Proveedor::orderBy($campo, $direc)->paginate($pag);
            elseif($status == 'trash'): 
                $provs = Proveedor::onlyTrashed()->orderBy($campo, $direc)->paginate($pag);
            endif;
        }
